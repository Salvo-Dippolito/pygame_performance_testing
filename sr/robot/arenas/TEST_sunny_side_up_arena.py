...
class TEST_SunnySideUpArena(Arena):
    size = (19, 10)
    start_locations = [( -8.3, -4.2)]
    ...
    def __init__(self, objects=None, wall_markers=True):
        super(TEST_SunnySideUpArena, self).__init__(objects, wall_markers)

        # file_path='home/salvatore/pippo_cleaned/python_simulator/robot-sim/sr/robot/arenas/silver_locations_file.txt'
        file_path=os.getcwd()+ '/sr/robot/arenas/silver_locations_file.txt'
        with open(file_path, 'r') as file:
            # Read the content and evaluate it as a Python expression
            content = file.read()
            # Use eval to convert the content to a Python object (list of tuples)
            silver_locations = eval(content)
    ...
    def draw_background(self, surface, display):
        super(TEST_SunnySideUpArena, self).draw_background(surface, display)
